...
> 公开仓库只包含合成演示数据和经过脱敏的核心源码。真实聊天、数据库、报告、API Key、私密安装包及原生聊天密钥恢复实现均未公开。
...
公开版刻意不包含 Windows 原生聊天密钥恢复代码。原因不是“没有做完”，而是：

1. 它依赖特定版本的第三方客户端内部实现；
2. 公开分发可能产生授权、平台条款和误用风险；
3. 面试作品的目标是展示产品判断与系统能力，而不是传播私人数据提取工具。

公开版保留 JSON 数据源接口、合成数据、全部产品层和本地数据架构。详细边界见 [Privacy & Security](docs/privacy-and-security.md)。
